export default {
    ...
    getters: {
        todo: state => {
            let result = { title: state.title, desc: state.desc, content: state.content };
            if (state.id != null) {
                result.id = state.id;
    ...
    actions: {
        ...
        deleteToDo({ commit, dispatch }, index) {
            ...
            commit("toggleToDo");
            dispatch("callTodoDelete").then(function (res) {
        ...
        insertTodo({ dispatch, commit, getters }) {
            ...
            dispatch("callTodoInsert").then(function (res) {
                if (res.status == 200 && res.data.status == 200) {
                    opts.type = "success";
        ...
        updateTodo({ dispatch, commit, getters }) {
            ...
            dispatch("callTodoUpdate").then(function (res) {
                if (res.status == 200 && res.data.status == 200) {
                    opts.type = "success";
        ...
        getTodoList({ dispatch, commit },) {
            ...
            dispatch("callTodoList").then(function (res) {
                if (res.status == 200 && res.data.status == 200) {
                    commit("setAppLoadingText", "getTodoList_success");
                    commit("setToDoList", res.data.data.map(v => {
                        v.checked = false;
                        return v;
                    }));
                } else {
